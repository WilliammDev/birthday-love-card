...
interface MemoryCakeProps {
  ...
  enableBlowAnimation?: boolean;
}

const MemoryCake = ({ onClick, hidePhoto, candlesBlown = false, enableBlowAnimation = false }: MemoryCakeProps) => {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [wishMade, setWishMade] = useState(candlesBlown);

  useEffect(() => {
    setWishMade(candlesBlown);
  }, [candlesBlown]);
  ...
  const handleWish = () => {
    if (!wishMade) {
      ...
      // Only show blow animation if enabled (Screen 2b)
      if (enableBlowAnimation) {
        // Show the blown-out candles for a moment before transitioning
        setTimeout(() => {
          onClick();
        }, 800); // 800ms delay to see the smoke animation
      } else {
        // Immediate transition (Screen 2a)
        onClick();
      }
    }
  }
  ...
  return (
    ...
    >
      ...
      {!hidePhoto && (
        <div className="w-11/12 transform -rotate-2 transition-transform duration-500 group-hover:rotate-0 group-hover:scale-105">
      ...
      <div className="flex flex-col items-center mt-2 transition-transform duration-300 group-hover:scale-105">
        {!wishMade && (
          <h3 className="font-dancing text-lg sm:text-xl text-rose-600 group-hover:text-purple-700 transition-colors animate-pulse-gentle drop-shadow-md">
            Ước một điều đi!
          </h3>
        )}
        <svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" className="w-32 h-32 -mt-2 drop-shadow-lg opacity-95">
          <defs>
